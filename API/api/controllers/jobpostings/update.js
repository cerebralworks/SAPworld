...
module.exports = function create(request, response) {
    ...
    schema.validate(post_request_data, { abortEarly: false }).then(async value => {
		...
		AND (COALESCE(user_profile.experience) >= job_posting.experience) AND (COALESCE(user_profile.experience) <= 2 ) group by user_profile.id ,locations.country`
					}else{
						var Count_Users = `SELECT  locations.country as "job_country",locations.id as "location_id",user_profile.* as "job_id" FROM user_employments "job_posting"
		CROSS JOIN user_profiles "user_profile" 
		LEFT JOIN users "user_account" ON (user_account.id=user_profile.account) 
		...
		AND (COALESCE(user_profile.experience) >= job_posting.experience) group by user_profile.id ,locations.country`
					}else{
						var Count_Users = `SELECT  locations.country as "job_country",locations.id as "location_id",user_profile.* as "job_id" FROM user_employments "job_posting"
		CROSS JOIN user_profiles "user_profile" 
		LEFT JOIN users "user_account" ON (user_account.id=user_profile.account) 
