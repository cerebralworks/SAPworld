    ...
    yup.object().shape({
	   ...
	   if(model.skills){
        ...
        if (model.city && model.visa_sponsorship == false && !value.user_id) {
			list_query.where(`((citys->>'city') = ANY( '{${model.city.toString()}}'))`);
        }
        if (model.country && model.visa_sponsorship == false && !value.user_id) {
            list_query.where(`((coun->>'country') = ANY( '{${model.country.toString()}}'))`);
        }
        ...
        if (model.visa_sponsorship == true && !value.user_id) {
            list_query.where(`(${UserProfiles.tableAlias}.${UserProfiles.schema.work_authorization.columnName} = 1 or  ((coun->>'country') = ANY( '{${model.country.toString()}}') AND (citys->>'city') = ANY( '{${model.city.toString()}}') ))`);
        }
