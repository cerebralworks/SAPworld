...
module.exports = async function apply(request, response) {
    ...
    const sendResponse = (details,logged_in_user,job,employee) => {
		...
		postDetails.message='You have a new applicant '+postDetails.name+' for the post /'+job.title +' / (' + job.job_locations[0].country +')';
		...
		postDetailss.message='Your application for '+postDetails.name+' is under review ';
