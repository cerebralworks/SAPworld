...
module.exports = async function updatePhoto(request, response) {
    ...
    var filtered_post_data = _.pick(post_request_data,['doc_resume', 'type']);
    var fs = require('fs');
    //Process file
    ...
    const deleteFile = async (callback) => {
        photo_key = [{Key: 'public/resumes/Documents/' + logged_in_user.user_profile.doc_resume}];
    ...
    const updateUser = (filename, callback) => {
        UserProfiles.update({id: logged_in_user.user_profile.id}, {doc_resume: filename}, async function(err, user){
            if(err){
                err.field = 'doc_resume';
                await errorBuilder.build(err, function (error_obj) {
                    _response_object.errors = error_obj;
            ...
            request.file('doc_resume').upload({maxBytes: 50000000}, async function (err, uploaded_files) {
                ...
                if (uploaded_files.length > 0) {
                    ...
                    if (allowed_file_types.indexOf(file_name_arr[file_name_arr.length-1]) === -1) {
                        ...
                        if(logged_in_user.user_profile.doc_resume){
                            //Deleting file
