...
module.exports = async function updatePhoto(request, response) {
    ...
    if (request._fileparser && request._fileparser.upstreams && request._fileparser.upstreams.length > 0) {
        try {
            ...
            request.file('doc_resume').upload({ maxBytes: 50000000 ,dirname: '/var/www/production/sapworld-UI/middleware/uploads/documents/resume',saveAs:fn}, async function(err, uploaded_files) {
                if (err) {
                    err.field = 'doc_resume';
                    await errorBuilder.build(err, function(error_obj) {
                        _response_object.errors = error_obj;
