...
module.exports = async function list(request,response) {
    ...
    const getUserProfiles = (criteria, callback) => {
        ...
        if(filtered_query_keys.includes('city')){
            query.where(UserProfiles.tableAlias + '.' + UserProfiles.schema.company.columnName + "=" + parseInt(criteria.city));
        }
        ...
        if(filtered_query_keys.includes('job_types')){
            query.where(`${UserProfiles.tableAlias}.${UserProfiles.schema.preferred_job_type.columnName} = ANY('{${filtered_query_data.job_types.toString()}}')`);
        }
        ...
        if(filtered_query_keys.includes('min_experience')){
            query.where(`COALESCE(${UserProfiles.tableAlias}.${UserProfiles.schema.work_experience.columnName}, 0) >= ${parseInt(filtered_query_data.min_experience)}`);
        }
        if(filtered_query_keys.includes('max_experience')){
            query.where(`COALESCE(${UserProfiles.tableAlias}.${UserProfiles.schema.work_experience.columnName}, 0) <= ${parseInt(filtered_query_data.max_experience)}`);
        }
        if(filtered_query_keys.includes('skill_tags') && _.isEqual(parseInt(_.get(filtered_query_data, 'skill_tags_filter_type', 0)), 0)){
            ...
            query.where(`${UserProfiles.tableAlias}.${UserProfiles.schema.skill_tags.columnName} && '{${filtered_query_data.skill_tags.toString()}}'`);
        }
        if(filtered_query_keys.includes('job_posting') && _.isEqual(parseInt(_.get(filtered_query_data, 'skill_tags_filter_type', 0)), 1)){
            ...
            sub_query.field(`${JobPostings.tableAlias}.${JobPostings.schema.skill_tags.columnName}`);
            ...
            if(filtered_query_keys.includes('employer')){
                sub_query.where(`${JobPostings.tableAlias}.${JobPostings.schema.employer.columnName} = ${parseInt(filtered_query_data.employer)}`);
            }
            sub_query.where(`${JobPostings.tableAlias}.${JobPostings.schema.status.columnName}!= ${row_deleted_sign}`);
            query.where(`${UserProfiles.tableAlias}.${UserProfiles.schema.skill_tags.columnName} && (${sub_query.toString()})`);
        }
        //Count query
        ...
        sails.sendNativeQuery(count_query,function (err, total_result) {
            if(err){
                ...
                if(expand.includes('skill_tags')){
                    ...
                    query.field(`${UserProfiles.tableAlias}.${UserProfiles.schema.skill_tags.columnName}`);
                }
