...
module.exports = async function list(request, response) {
	...
	(user_account.status=1) AND ((LOWER(user_profile.country) like job_posting.country) 
	or (coun->>'country') like job_posting.country  or (LOWER(user_profile.city) like job_posting.city)
	or (citys->>'city') like job_posting.city) AND (user_profile.privacy_protection->>'available_for_opportunity')::text = 'true' AND (user_profile.skills && ARRAY[skillss->>'skill_id']::bigint[])
