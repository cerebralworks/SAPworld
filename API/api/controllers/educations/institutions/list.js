...
module.exports = async function list(request,response) {
    ...
    //Find the EducationalInstitutions based on general criteria.
    const getEducationalInstitutions = (criteria, callback) => {
        EducationalInstitutions.count(criteria.where, function (err, total) {
            if(err){
                var error = {
                    'institution': 'count',
                    'rules': [
                ...
                var educational_institution_model = EducationalInstitutions.find(criteria); 
                educational_institution_model.exec(async function(err, educational_institutions){
                    if(err){
                        var error = {
                            'institution': 'items',
                            'rules': [
                        ...
                        return callback(educational_institutions, {}, total);
                    }
                });
            }
        });
    ...
    const sendResponse = (items, details, total) => {
        _response_object.message = 'Educational institution items retrieved successfully.';
        var meta = {};
    ...
    validateModel.validate(null, input_attributes, filtered_query_data, async function(valid, errors){
        if(valid){
            ...
            //Preparing data.
            await getEducationalInstitutions(criteria, function (educational_institutions, details, total) {
                sendResponse(educational_institutions, details, total);
            });
        }
        else{
