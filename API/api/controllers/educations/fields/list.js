...
module.exports = async function list(request,response) {
    ...
    //Find the EducationalFields based on general criteria.
    const getEducationalFields = (criteria, callback) => {
        EducationalFields.count(criteria.where, function (err, total) {
            if(err){
                var error = {
                    'field': 'count',
                    'rules': [
                ...
                var educational_field_model = EducationalFields.find(criteria); 
                educational_field_model.exec(async function(err, educational_fields){
                    if(err){
                        var error = {
                            'field': 'items',
                            'rules': [
                        ...
                        return callback(educational_fields, {}, total);
                    }
                });
            }
        });
    ...
    const sendResponse = (items, details, total) => {
        _response_object.message = 'Educational field items retrieved successfully.';
        var meta = {};
    ...
    validateModel.validate(null, input_attributes, filtered_query_data, async function(valid, errors){
        if(valid){
            ...
            if(filtered_query_keys.includes('status')) {
                ...
                // We should exclude deleted educational fields.
            ...
            //Preparing data.
            await getEducationalFields(criteria, function (educational_fields, details, total) {
                sendResponse(educational_fields, details, total);
            });
        }
        else{
