...
module.exports = async function list(request,response) {
    ...
    //Find the SubscriptionPlans based on general criteria.
    const getSubscriptionPlans = (criteria, callback) => {
        SubscriptionPlans.count(criteria.where, function (err, total) {
            if(err){
                var error = {
                    'field': 'count',
                    'rules': [
                ...
                var subscription_plan_model = SubscriptionPlans.find(criteria); 
                subscription_plan_model.exec(async function(err, subscription_plans){
                    if(err){
                        var error = {
                            'field': 'items',
                            'rules': [
                        ...
                        return callback(subscription_plans, {}, total);
                    }
                });
            }
        });
    ...
    const sendResponse = (items, details, total) => {
        _response_object.message = 'Subscription plan items retrieved successfully.';
        var meta = {};
    ...
    validateModel.validate(null, input_attributes, filtered_query_data, async function(valid, errors){
        if(valid){
            ...
            if(filtered_query_keys.includes('status')) {
                ...
                // We should exclude deleted subscription plans.
            ...
            //Preparing data.
            await getSubscriptionPlans(criteria, function (subscription_plans, details, total) {
                sendResponse(subscription_plans, details, total);
            });
        }
        else{
