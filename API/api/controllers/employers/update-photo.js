   ...
   /* const uploadPhotos = async (photo,callback) => {
    ...
    if (request._fileparser && request._fileparser.upstreams && request._fileparser.upstreams.length > 0) {
        try{
            ...
            request.file('photo').upload({maxBytes: 50000000 ,dirname: '/var/www/production/sapworld-UI/middleware/uploads/images/employer',saveAs:fn}, async function (err, uploaded_files) {
                if (err) {
                    err.field = 'photo';
                    await errorBuilder.build(err, function (error_obj) {
                        _response_object.errors = error_obj;
                ...
                if (uploaded_files.length > 0) {
					...
					/*let filename = fn;
					  let uploadLocation =require('path').resolve(process.cwd(),'uploads/images/employer/' + filename);
