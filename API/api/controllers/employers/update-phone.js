...
module.exports = function updatePhone(request, response) {
    ...
    validateModel.validate(null, input_attributes, filtered_post_data, async function(valid, errors){
        if(valid){
                ...
                EmployerProfiles.update({id: logged_in_user.employer_profile.id}, {phone: filtered_post_data.encrypted_phone}, async function(err, user){
                    if(err){
                    ...
                    }else if(logged_in_user.username === logged_in_user.employer_profile.phone){
