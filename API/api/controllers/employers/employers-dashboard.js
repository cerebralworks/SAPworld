...
LEFT JOIN user_employments "job_posting" ON (job_posting.id=job_applicationsss.job_posting) 
 ...
 (SELECT COUNT(DISTINCT job_applicationsss.id) FROM  job_applications "job_applicationsss" 
LEFT JOIN user_employments "job_posting" ON (job_posting.id=job_applicationsss.job_posting) 
 WHERE job_posting.id = job_postings.id  ${filterUpdatedDate}  AND (job_applicationsss.short_listed = true AND (job_applicationsss.status =  9 OR job_applicationsss.status =  2) )   AND (job_applicationsss.employer=${parseInt(filtered_query_data.id)} ) ) as accepted,
...
LEFT JOIN user_employments "job_posting" ON (job_posting.id=job_applicationsss.job_posting) 
 ...
 (SELECT COUNT(DISTINCT job_applicationsss.id) FROM  job_applications "job_applicationsss" 
LEFT JOIN user_employments "job_posting" ON (job_posting.id=job_applicationsss.job_posting) 
 WHERE job_posting.id = job_postings.id  ${filterUpdatedDate}  AND (job_applicationsss.short_listed = true AND (job_applicationsss.status =  3 OR job_applicationsss.status =  14 OR job_applicationsss.status =  12  ))   AND (job_applicationsss.employer=${parseInt(filtered_query_data.id)} ) ) as interviewed,
