...
module.exports = async function dashboardDetails(request, response) {
    ...
    const filtered_query_data = _.pick(request_query, ['page','status','id', 'sort', 'column', 'limit','company','view']);
    ...
    const getListDatas = async( callback) => {
			query = `SELECT job_posting.title as title,job_posting.created_at as created_at,
			...
			where job_posting.status != 3 ORDER BY ${filtered_query_data.column} ${filtered_query_data.sort}
			...
			FROM user_employments "job_posting" 
			LEFT JOIN employer_profiles "employer" ON (job_posting.company = employer.id)
			where job_posting.status != 3 AND job_posting.status != 0 ORDER BY ${filtered_query_data.column} ${filtered_query_data.sort}
			 LIMIT ${filtered_query_data.limit} OFFSET ${filtered_query_data.page} `	
			 
			queryCount =`SELECT count(*)
			FROM user_employments "job_posting" 
			LEFT JOIN employer_profiles "employer" ON (job_posting.company = employer.id)
			where job_posting.status != 3 AND job_posting.status != 0  `	
				}
			...
			FROM user_employments "job_posting" 
			LEFT JOIN employer_profiles "employer" ON (job_posting.company = employer.id)
			where job_posting.status != 3 AND job_posting.status != 1 ORDER BY ${filtered_query_data.column} ${filtered_query_data.sort}
			 LIMIT ${filtered_query_data.limit} OFFSET ${filtered_query_data.page}`	
			 
			queryCount =`SELECT count(*)
			FROM user_employments "job_posting" 
			LEFT JOIN employer_profiles "employer" ON (job_posting.company = employer.id)
			where job_posting.status != 3 AND job_posting.status != 1 `	
				}
			...
			query =`SELECT user_profile.first_name,user_profile.last_name,user_profile.created_at,
				...
				FROM  user_profiles "user_profile" ORDER BY ${filtered_query_data.column} ${filtered_query_data.sort}
			 LIMIT ${filtered_query_data.limit} OFFSET ${filtered_query_data.page}`	
			...
			query =`SELECT user_profile.first_name,user_profile.last_name,user_profile.created_at,
				user_profile.city,user_profile.state,user_profile.country,user_profile.job_role
				FROM  user_profiles "user_profile" where 
				 ((user_profile.privacy_protection->>'available_for_opportunity')::text = 'true' ) 
