...
module.exports = async function Details(request, response) {
    ...
    const getNotification = async( callback) => {
		...
		if(filtered_query_data.view =='user'){
			...
			Count_Users = `SELECT job_posting.status as jobstatus, notification.view,notification.created_at,notification.title,notification.account,notification.user_id,notification.updated_at,notification.employer,notification.id,notification.job_id,notification.message,notification.name,notification.status FROM notifications as notification LEFT JOIN user_employments "job_posting" ON (notification.job_id = job_posting.id) where notification.account = ${logged_in_user.id}  ORDER BY notification.id DESC LIMIT ${filtered_query_data.limit} OFFSET ${parseInt(filtered_query_data.limit)*parseInt(filtered_query_data.page)}`
