...
/* global _, validateModel, Cities */

module.exports = async function list(request,response) {
    var _response_object = {};
    const request_query = request.allParams();
    const filtered_query_data = _.pick(request_query, ['page', 'sort', 'limit', 'search', 'status', 'expand', 'state', 'country']);
    ...
    //Find the Cities based on general criteria.
    const getCities = (criteria, callback) => {
        Cities.count(criteria.where,function (err, total) {
            if(err){
                var error = {
                    'field': 'count',
                    'rules': [
                ...
                var city_model = Cities.find(criteria);
                if(expand.includes('state')){
                    city_model.populate('state');
                }
                city_model.exec(async function(err, cities){
                    if(err){
                        var error = {
                            'field': 'items',
                            'rules': [
    ...
    const sendResponse = (items, details, total) => {
        _response_object.message = 'City items retrieved successfully.';
        var meta = {};
    ...
    validateModel.validate(null, input_attributes, filtered_query_data, async function(valid, errors){
        if(valid){
            ...
            //Preparing data
            await getCities(criteria, function (cities, details, total) {
                sendResponse(cities, details, total);
            });
        }
        else{
