...
/* global _, validateModel, States */

module.exports = async function list(request,response) {
    var _response_object = {};
    const request_query = request.allParams();
    const filtered_query_data = _.pick(request_query, ['page', 'sort', 'limit', 'search', 'status', 'country']);
    ...
    //Find the States based on general criteria.
    const getStates = (criteria, callback) => {
        States.count(criteria.where,function (err, total) {
            if(err){
                var error = {
                    'field': 'count',
                    'rules': [
                ...
                var state_model = States.find(criteria);
                state_model.exec(async function(err, states){
                    if(err){
                        var error = {
                            'field': 'items',
                            'rules': [
    ...
    const sendResponse = (items, details, total) => {
        _response_object.message = 'State items retrieved successfully.';
        var meta = {};
    ...
    validateModel.validate(null, input_attributes, filtered_query_data, async function(valid, errors){
        if(valid){
            ...
            //Preparing data
            await getStates(criteria, function (states, details, total) {
                sendResponse(states, details, total);
            });
        }
        else{
