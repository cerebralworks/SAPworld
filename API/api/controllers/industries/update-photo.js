...
module.exports = async function updatePhoto(request, response) {
    ...
    const Industries_id = request.allParams().id || 0;
    ...
    const uploadPhotos = async(photo, callback) => {
        path = require('path');
        filename = 'photo-' + Industries_id + path.extname(photo.filename);
        file_path = 'public/images/Industry';
    ...
    const updateIndustry = (filename, callback) => {
        Industries.update({ id: Industries_id }, { photo: filename }, async function(err, user) {
            if (err) {
                err.field = 'photo';
                await errorBuilder.build(err, function(error_obj) {
                    _response_object.errors = error_obj;
                ...
                if (uploaded_files.length > 0) {
                    ...
                    if (allowed_file_types.indexOf(uploaded_files[0].type) === -1) {
                        ...
                        await uploadPhotos(uploaded_files[0], async function(filename) {
                            //Update user
                            await updateIndustry(filename, function(details) {
