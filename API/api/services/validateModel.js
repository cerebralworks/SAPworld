    ...
    async.forEachOf(input_attributes, function (value, key, callback){
        try {
            if(parseInt(post_request_data[value.name]) !== 0 && post_request_data[value.name] !== false && !post_request_data[value.name] && value.required){
                ...
                if(value.phone){
                    if(!validator.isMobilePhone(post_request_data[value.name].toString(),'en-US')){
                      var error_obj = new Error();
                      error_obj.code = "E_VIOLATES_RULES";
                      if(value.message){
                          error_obj.message = value.message;
                ...
                if(value.isdate){
                    var valid = (new Date(post_request_data[value.name])).getTime() > 0;
                    if(!valid)
                    var error_obj = new Error();
                    error_obj.code = "E_VIOLATES_RULES";
                    if(value.message){
                        ...
                        error_obj.message = value.parent_name + " must be a date.";
                    }else{
                        error_obj.message = value.name + " must be a date.";
                    }
                ...
                if(value.custom){
                    if(!value.result){
                      ...
                      error_obj.ruleViolations = [{rule: 'custom', message: error_obj.message}];
                      throw error_obj;
                    }
                }
              }
